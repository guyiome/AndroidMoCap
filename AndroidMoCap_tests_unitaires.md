...
| `EyeBlinkCorrection.kt` | `earOpenness()`, `correctBlinkScore()`, `EyeOpennessSmoother`, `correctEyeBlinkScores()` | `EyeBlinkCorrectionTest.kt` | ✅ Couvert (13 tests -- atténuation d'une fuite gauche/droite vs passage inchangé d'une vraie fermeture, lissage asymétrique attaque rapide/relâchement lent dont la régression exacte du bug "effondrement en tenue longue", orchestration bout en bout). Corrige le blendshape `eyeBlinkLeft/Right` avant tout envoi réseau -- voir revue technique, point 28. `EyeOpennessSmoother` a vocation à être remplacé par `OneEuroFilter.kt` ci-dessous, pas encore fait -- voir point 46. |
| `OneEuroFilter.kt` | Filtre passe-bas à coupure adaptative (`minCutoff`, `beta`, `dCutoff`) | `OneEuroFilterTest.kt` | ✅ Couvert (8 tests -- premier échantillon non lissé, signal constant stable, saut isolé amorti, `beta` élevé réduit le retard sur un mouvement soutenu, `minCutoff` bas lisse davantage un signal bruité, convergence sur une cible tenue). Outil générique construit mais pas encore intégré au pipeline -- voir revue technique, point 46. |
...
20 fichiers de test, tous en JVM pur (aucun appareil/émulateur requis) : `RotationMathTest`, `TrackingTierSelectorTest`, `BlendshapeCatalogTest`, `FaceLandmarkerHelperTest`, `IFacialMocapSenderTest`, `CameraControllerTest`, `VmcOscSenderTest`, `LandmarkProjectionTest`, `ArCoreFaceSelectorTest`, `MeshOverlayVisibilityTest`, `ThermalThrottleTest`, `DebugPanelUnlockTest`, `CalibrationAnomalyTest`, `BlendshapeStabilityTest`, `VTubeStudioProtocolTest`, `VTubeStudioConnectionStateTest`, `AppLanguageTest`, `EyeAspectRatioTest`, `EyeBlinkCorrectionTest`, `OneEuroFilterTest`. Ils couvrent l'intégralité de la logique mathématique et de formatage identifiée comme la plus fragile (rotation/calibration, mapping de noms de blendshapes, regard des yeux, sélection de palier (dont son forçage manuel), dimensions de rotation caméra, regroupement des messages OSC, projection écran du mesh, sélection du visage principal ARCore, visibilité de l'overlay, réponse au throttling thermique, déverrouillage du panneau de mocks de debug, détection d'anomalie de calibrage, protocole et machine à état de l'API Plugin VTube Studio, mapping de la langue en-app, Eye Aspect Ratio et sa correction anti-fuite/lissage pour la fiabilisation du clignement, filtre à coupure adaptative générique), en s'appuyant sur quelques extractions de fonctions pures (visibilité `internal`) qui ne changent aucun comportement.
