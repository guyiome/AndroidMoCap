...
| `EyeBlinkCorrection.kt` | `earOpenness()`, `correctBlinkScore()`, `correctEyeBlinkScores()` | `EyeBlinkCorrectionTest.kt` | ✅ Couvert (9 tests -- atténuation d'une fuite gauche/droite vs passage inchangé d'une vraie fermeture, orchestration bout en bout dont une tenue face à une dérive réaliste). Corrige le blendshape `eyeBlinkLeft/Right` avant tout envoi réseau -- voir revue technique, point 28. Le lissage temporel (`EyeOpennessSmoother`, retiré) est maintenant `OneEuroFilter.kt` ci-dessous -- voir point 46. |
