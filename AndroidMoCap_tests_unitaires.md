...
|---|---|---|---|
| `RotationMath.kt` | `multiply`, `transpose`, `rotation3x3FromColumnMajor4x4`, `toEulerDegrees`, `mirrorEulerDegrees`, `composeCalibratedEuler`, `rotation3x3FromQuaternion` | `RotationMathTest.kt` | ✅ Couvert (15 tests) -- c'est la classe la plus délicate de l'app (origine de l'inversion d'axes déjà rencontrée), priorité n°1. `rotation3x3FromQuaternion` est la brique préparatoire à la fusion ARCore (rapport technique, point 13) : convertit le quaternion `Pose#getRotationQuaternion()` d'ARCore vers le même format 3x3 que le reste du pipeline. `toEulerDegrees` ne mirrore plus le yaw depuis le point 51 (comportement natif/anatomique) -- `mirrorEulerDegrees` porte désormais le mode miroir explicitement, testé en isolation. |
...
| `CalibrationAnomaly.kt` | `CalibrationAnomalyState.next()`, `maxAbsEulerDegrees()` | `CalibrationAnomalyTest.kt` | ✅ Couvert (fonction pure, aucune dépendance de timing réel -- comportement sticky, les deux signaux (repos+dérive, redétection de visage), gating repos/pose, rafales courtes qui ne déclenchent pas l'anomalie, expression active vs immobilité réelle, redétection indépendante du repos). Voir revue technique, point 19. |
| `BlendshapeStability.kt` | `meanAbsoluteBlendshapeDelta()` | `BlendshapeStabilityTest.kt` | ✅ Couvert (fonction pure -- sentinelle si comparaison impossible, delta correct, appariement par nom plutôt que par position, robuste à un ordre différent entre les deux listes). Voir revue technique, point 19. |
| `EyeAspectRatio.kt` | `eyeAspectRatio()`, `eyeAspectRatioFromLandmarks()` | `EyeAspectRatioTest.kt` | ✅ Couvert (fonction pure -- œil ouvert dans la plage attendue, œil fermé proche de zéro, garde division par zéro si coins confondus, lecture des bons indices depuis une liste de landmarks, liste trop courte -> 0). Utilisé par `EyeBlinkCorrection.kt` -- voir revue technique, point 28. |
| `EyeBlinkCorrection.kt` | `earOpenness()`, `correctBlinkScore()`, `correctEyeBlinkScores()` | `EyeBlinkCorrectionTest.kt` | ✅ Couvert (10 tests -- atténuation d'une fuite gauche/droite vs passage inchangé d'une vraie fermeture, orchestration bout en bout dont une tenue face à une dérive réaliste, et un clignement réel à angle de caméra inhabituel qui s'améliore après quelques épisodes). Corrige le blendshape `eyeBlinkLeft/Right` avant tout envoi réseau -- voir revue technique, point 28. Le lissage temporel (`EyeOpennessSmoother`, retiré) est maintenant `OneEuroFilter.kt` ci-dessous -- voir point 46. La référence "fermé" est auto-adaptative (`AdaptiveEarFloor`, ligne suivante) -- voir point 48. |
| `EyeBlinkCorrection.kt` | `AdaptiveEarFloor.next()` -- référence EAR "fermé" auto-adaptative par œil | `AdaptiveEarFloorTest.kt` | ✅ Couvert (6 tests -- valeur initiale, aucun changement sous le seuil d'activité sans épisode en cours, minimum retenu sur tout l'épisode même si l'EAR remonte en cours de route, mélange partiel à la fin de l'épisode, convergence sur plusieurs épisodes vers le minimum réel, épisode isolé pollué qui ne redéfinit pas la référence à lui seul). Corrige un clin d'œil réel écrasé à un angle de caméra en contre-plongée -- voir revue technique, point 48. |
| `BrowRaise.kt` | `browRaiseRatio()`, `browRaiseRatioFromLandmarks()` -- même principe que l'EAR appliqué aux sourcils | `BrowRaiseTest.kt` | ✅ Couvert (5 tests -- sourcil levé > abaissé, calcul sur géométrie connue, coins d'œil confondus -> zéro, lecture des bons indices, liste trop courte -> zéro). Correspondance gauche/droite des indices ([BrowLandmarkIndices]) déduite par analogie, **pas encore confirmée sur device** -- diagnostic temporaire en place (`BrowDiag`), voir revue technique. |
| `Mirroring.kt` | `mirrorBlendshapeName()`, `mirrorBlendshapes()`, `mirrorFaceTrackingResult()` -- mode miroir (tête + blendshapes gauche/droite + regard par œil) | `MirroringTest.kt` | ✅ Couvert (8 tests -- échange de noms, blendshapes/`FaceTrackingResult` sans paire inchangés, couverture systématique du vrai catalogue, involution). Corrige une incohérence réelle trouvée et confirmée sur device (tête mirrorée, blendshapes pas mirrorés) -- voir revue technique, point 51. |
| `OneEuroFilter.kt` | Filtre passe-bas à coupure adaptative (`minCutoff`, `beta`, `dCutoff`) | `OneEuroFilterTest.kt` | ✅ Couvert (8 tests -- premier échantillon non lissé, signal constant stable, saut isolé amorti, `beta` élevé réduit le retard sur un mouvement soutenu, `minCutoff` bas lisse davantage un signal bruité, convergence sur une cible tenue). Intégré dans `EyeBlinkCorrection.kt` et confirmé sur device -- voir revue technique, point 46. |
| `TongueOutGate.kt` | `jawOpenGateOpen()` -- étage 1 de la cascade langue tirée | `TongueOutGateTest.kt` | ✅ Couvert (5 tests -- sous/à/au-dessus du seuil, bornes 0f/1f). Confirmé fiable sur device (CameraX + ARCore) -- voir revue technique, point 15. |
| `LipLandmarks.kt` | `mouthOpennessRatio()`, `mouthCropRegion()` | `LipLandmarksTest.kt` | ✅ Couvert (8 tests -- géométrie connue, coins confondus -> null/0, liste trop courte, dimensions image invalides, clamp aux bornes). `LipLandmarkIndices` **confirmé sur device le 11 août 2026** (coordonnées brutes loguées et comparées -- mapping initial inversé, corrigé) -- voir revue technique, point 15. |
| `MouthColorAnalysis.kt` | `rgbToHsv()`, `isTongueColoredPixel()`, `tonguePixelRatio()`, `colorGateOpen()`, `averageHsv()` -- étage 2 | `MouthColorAnalysisTest.kt` | ✅ Couvert (20 tests, valeurs HSV dérivées à la main). Mécanique confirmée fonctionnelle sur device (CameraX + ARCore) mais le classifieur couleur lui-même **non fiable** en pratique (seuil dérive trop d'une session à l'autre) -- voir `TongueColorBaseline.kt` ci-dessous et revue technique, point 15. |
| `TongueColorBaseline.kt` | `TongueColorBaseline.next()`, `isElevated()` -- référence "sans langue" adaptative (même principe qu'`AdaptiveEarFloor`, point 48) | `TongueColorBaselineTest.kt` | ✅ Couvert (6 tests -- premier échantillon, dérive EMA en zone normale, pas de dérive en zone élevée, `isElevated` sous/au-dessus de la marge). Améliore la discrimination par rapport à un seuil fixe sans la rendre fiable -- voir revue technique, point 15. |
| `InferenceLoadMonitor.kt` | `InferenceLoadState.next()` (EMA), `isRunningHigh()` | `InferenceLoadMonitorTest.kt` | ✅ Couvert (6 tests -- premier échantillon brut, convergence EMA, `isRunningHigh` sous/au-dessus de la marge, faux avant le premier échantillon). Alimente l'avertissement de charge de la détection langue tirée -- voir revue technique, point 15. |

## logging/

| Fichier | Élément couvert | Test(s) | Statut |
|---|---|---|---|
| `LogFormatting.kt` | `formatLogLine()`, `maskIpAddresses()`, `appendWithRotation()`, `shouldPersist()` | `LogFormattingTest.kt` | ✅ Couvert (13 tests -- format de ligne avec/sans pile d'appel, masquage IPv4 simple/intégré à un message/multiple/absent, rotation sous et au-delà de la limite de taille (garde les entrées récentes, jamais de ligne partielle en tête), filtrage par seuil de niveau). Voir revue technique, point 50. |
...
| `VmcOscSender.kt` | `buildBundle()`, `serializeToBytes()` (extraites `internal`, pures) | `VmcOscSenderTest.kt` | ✅ Couvert -- `buildBundle()` regroupe les messages `/Val` + `/Apply` en un seul `OSCBundle` (voir rapport technique, point 4). `serializeToBytes()` (ajoutée au point 38, correctif du crash `NoSuchMethodError` sur Android 11/API 30) est testée en round-trip complet : sérialise un bundle puis le reparse avec `OSCParser` et compare le contenu -- preuve en JVM que le remplacement d'`OSCPortOut` produit des octets valides, pas seulement qu'il compile. |
| `VmcOscSender.kt` | `send()` (partie socket), `connect()` | -- | ❌ Dépend d'un vrai `DatagramSocket`/UDP -- nécessite un test instrumenté ou un socket factice. La partie à risque (le contenu du bundle ET sa sérialisation en octets) est déjà couverte séparément via `serializeToBytes()`. |
...
| `CameraController.kt` | `bindImageAnalysis()`, `bindPreview()`, `processFrame()`, `acquirePooledBitmap()`, `releaseFrame()`, `peekPooledBitmap()` | -- | ❌ Dépend de CameraX (`ProcessCameraProvider`, `ImageProxy`) et `android.graphics.Bitmap/Canvas/Matrix` -- nécessite un test instrumenté (CameraX fournit un `CameraXConfig` de test, mais ça reste un test sur device/émulateur) ou Robolectric avec ses shadows graphiques. Le pool de bitmaps a déjà causé un bug réel (comparaison par identité `MPImage`, corrigé) : bon candidat à un test instrumenté si l'app grossit encore sur ce point. `peekPooledBitmap()` (point 15) alimente l'étage 2 de la cascade langue tirée sur palier CameraX. |
| `ArCoreHeadPoseTracker.kt` | Session ARCore, conversion YUV->Bitmap, `peekLastBitmap()`, `releaseFrame()` | -- | ❌ Dépend d'ARCore (`Session`, `Frame`) et `GLSurfaceView.Renderer` -- même limite que `CameraController.kt`, nécessite un test instrumenté. `peekLastBitmap()`/`releaseFrame()` (point 15, ajoutés le 11 août 2026) donnent à l'étage 2 de la cascade langue tirée un accès pixel sur palier ARCore, qui n'avait auparavant aucun pool de bitmap. **Bug de rotation caméra confirmé sur device séparément** (image tournée ~90° anti-horaire) -- voir revue technique, point 15, section phase 1. |
...
| `DebugPanelUnlock.kt` | `DebugPanelUnlockState.registerTap()` | `DebugPanelUnlockTest.kt` | ✅ Couvert (fonction pure, aucune dépendance Android -- comptage de taps consécutifs dans une fenêtre de temps, reset hors fenêtre, borne inclusive, no-op une fois déverrouillé). Voir revue technique, point 35. |

## Racine

| Fichier | Élément couvert | Test(s) | Statut |
|---|---|---|---|
| `MainActivity.kt`, `MoCapApplication.kt` | Cycle de vie Android (`MoCapApplication.onCreate()` initialise aussi `AppLog`, voir point 50 -- un appel direct sans logique propre à isoler) | -- | ❌ Points d'entrée Android purs, testables uniquement via test instrumenté. |

## Bilan actuel

29 fichiers de test, tous en JVM pur (aucun appareil/émulateur requis) : `RotationMathTest`, `TrackingTierSelectorTest`, `BlendshapeCatalogTest`, `FaceLandmarkerHelperTest`, `IFacialMocapSenderTest`, `CameraControllerTest`, `VmcOscSenderTest`, `LandmarkProjectionTest`, `ArCoreFaceSelectorTest`, `MeshOverlayVisibilityTest`, `ThermalThrottleTest`, `DebugPanelUnlockTest`, `CalibrationAnomalyTest`, `BlendshapeStabilityTest`, `VTubeStudioProtocolTest`, `VTubeStudioConnectionStateTest`, `AppLanguageTest`, `EyeAspectRatioTest`, `EyeBlinkCorrectionTest`, `OneEuroFilterTest`, `AdaptiveEarFloorTest`, `LogFormattingTest`, `BrowRaiseTest`, `MirroringTest`, `TongueOutGateTest`, `LipLandmarksTest`, `MouthColorAnalysisTest`, `TongueColorBaselineTest`, `InferenceLoadMonitorTest`. Ils couvrent l'intégralité de la logique mathématique et de formatage identifiée comme la plus fragile (rotation/calibration -- dont le mode miroir, mapping de noms de blendshapes, regard des yeux, sélection de palier (dont son forçage manuel), dimensions de rotation caméra, regroupement des messages OSC, projection écran du mesh, sélection du visage principal ARCore, visibilité de l'overlay, réponse au throttling thermique, déverrouillage du panneau de mocks de debug, détection d'anomalie de calibrage, protocole et machine à état de l'API Plugin VTube Studio, mapping de la langue en-app, Eye Aspect Ratio et sa correction anti-fuite/lissage pour la fiabilisation du clignement, filtre à coupure adaptative générique, référence EAR "fermé" auto-adaptative par œil, formatage/masquage/rotation des logs, hauteur géométrique des sourcils, cascade de détection de la langue tirée -- porte jawOpen, recadrage buccal, analyse couleur, référence couleur adaptative, moniteur de charge d'inférence), en s'appuyant sur quelques extractions de fonctions pures (visibilité `internal`) qui ne changent aucun comportement.
