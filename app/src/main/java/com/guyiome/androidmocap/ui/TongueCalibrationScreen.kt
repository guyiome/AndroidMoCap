 ...
 *
 * **Deux bugs successifs identifiés et corrigés sur retours device (paysage) :**
 * 1. `graphicsLayer(rotationZ = ...)` seul pivote autour du **centre** du bloc, alors que
 *    `align(...)` le positionne selon sa forme **non tournée** -- un bloc large-et-bas (le message)
 *    et un bloc étroit-et-haut (le bouton) dérivent chacun différemment de leur ancrage une fois
 *    tournés, selon leur propre ratio largeur/hauteur (bloc "à mi-hauteur, à moitié découpé", bouton
 *    "en plein milieu de l'écran").
 * 2. Tentative suivante : faire pivoter autour du même coin que l'ancrage plutôt que du centre
 *    (`transformOrigin` custom). Toujours faux -- sans savoir dans quel sens le rectangle pivote
 *    autour de ce coin (question de signe/sens de rotation, facile à se tromper sans device pour
 *    vérifier), le bloc peut tout aussi bien pivoter hors-écran que dedans (constaté : bloc quasi
 *    entièrement invisible, seul un bord visible dans un coin -- exactement ce que donne un
 *    rectangle qui pivote du mauvais côté de son pivot).
 *
 * Fix retenu, plus robuste : [RotatedBox] réserve dans la mise en page du parent la forme **déjà
 * tournée** (largeur/hauteur échangées pour ±90°) et centre le contenu non tourné à l'intérieur avant
 * d'appliquer la rotation -- la rotation remplit alors exactement l'espace annoncé au parent, donc
 * `align(...)` en amont fonctionne aussi simplement que pour un élément jamais tourné, sans plus
 * jamais avoir à raisonner sur un pivot ou un sens de rotation.
 */
/**
 * Fait pivoter [content] de [rotationDegrees] (paliers 0/±90/180, voir [snapToRotationBucket]) tout
 * en réservant, dans la mise en page du parent, l'espace correspondant à sa forme **après** rotation
 * (largeur/hauteur échangées pour ±90°) plutôt que sa forme d'origine -- voir kdoc de
 * [TongueCalibrationScreen] pour le bug que ça corrige. Mesure [content] à sa taille naturelle, le
 * centre dans une boîte dont la taille annoncée au parent est déjà permutée, puis pivote autour de ce
 * centre commun : la rotation remplit alors exactement la boîte annoncée, donc `Modifier.align(...)`
 * appliqué en amont sur ce composable se comporte normalement, sans notion de pivot à gérer à
 * l'appel.
 */
@Composable
private fun RotatedBox(
    ...
    Layout(
        ...
        val swapped = rotationDegrees == 90f || rotationDegrees == -90f
        val layoutWidth = if (swapped) placeable.height else placeable.width
        val layoutHeight = if (swapped) placeable.width else placeable.height
...
fun TongueCalibrationScreen(
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        // voir kdoc de la fonction. `align(...)` se place ici sur le Box extérieur (marge d'écran
        // fixe, jamais tourné) ; [RotatedBox] à l'intérieur gère la rotation elle-même.
        //
        ...
        Box(
            ...
            RotatedBox(rotationDegrees = contentRotationZ) {
                Column(
                    modifier = Modifier
                    ...
                    when {
                        // Confirmation de fin, affichée seule (ni étapes à venir, ni bouton en
                        // dessous) pendant CALIBRATION_DONE_DISPLAY_MS avant la fermeture automatique.
                        justFinished -> {
                            Text(
                        ...
                        phase == TongueCalibrationPhase.IDLE || phase == TongueCalibrationPhase.DONE -> {
                            ...
                            // ExperimentalFeaturesScreen) -- un déroulé court des étapes à venir
                            // remplace le paragraphe d'origine, retour utilisateur explicite.
                            Text(
                            ...
                            val statusText = when (phase) {
                                ...
                                // IDLE/DONE exclus par la condition du when englobant -- smart-cast,
                                // when exhaustif sans eux, pas de else nécessaire.
                            }
                            Text(
                                statusText,
                                color = Color.White,
                    ...
                    // positionner, un repère bien visible aide), petits pendant la prise de vues
                    // elle-même (ne doit pas distraire du geste à tenir) -- demande explicite de
                    // l'utilisateur. Masqué pendant justFinished (secondsRemaining est déjà null à ce
        ...
        // Même structure Box+RotatedBox que le message (align(...) sur l'extérieur, jamais tourné ;
        // RotatedBox gère la rotation à l'intérieur).
                    ...
                    Column(
                        ...
                        if (debugUnlock.unlocked) {
                            Column(
                                ...
                                // Unité de base, pas la durée réelle d'une phase (chaque phase la
                                // multiplie par son propre facteur -- voir
                                // TONGUE_OUT_RECORDING_MULTIPLIER/TONGUE_IN_RECORDING_MULTIPLIER) --
                                // pas de rebuild pour retenter un réglage.
                                Spacer(Modifier.height(8.dp))
                                Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.fillMaxWidth()) {
                                    Text(
                                ...
                                // Marge de classification (voir
                                // TongueEmbeddingClassifier.classifyTongueState).
